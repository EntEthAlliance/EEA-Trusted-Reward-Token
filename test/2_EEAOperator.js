...
contract('EEAOperator', (accounts) => {
    ...
    describe('mint penalties', function () {
        ...
        describe('when the sender is the EEA issuer', function () {
            describe('When the recipients are EEA members', function () {
                describe('but Issuer sets the wrong organization/employee match', function () {
                    it('reverts', async function () {
                        await expectThrow(eeaOperatorInstance.mintPenalties(organization2, to, amount, '0x0', {
                            from: eeaAdmin
                        }));
                    });
                });
                describe('and reputation is burned for employee/organization, penalties for organization', function () {
                    it('it succeeds', async function () {
                        ...
                        await eeaOperatorInstance.mintPenalties(org, to, amount, '0x0', {
                            from: eeaAdmin
                        });
                        const memberBalanceRep = await reputationTokenInstance.balanceOf(to);
                        const memberBalancePenalties = await penaltyTokenInstance.balanceOf(to);
                        const orgBalanceRep = await reputationTokenInstance.balanceOf(org);
                        const orgBalancePenalties = await penaltyTokenInstance.balanceOf(org);

                        ((oldRepBalanceEmployee.sub(oneToken)).toString()).should.be.equal(memberBalanceRep.toString());
                        ((oldRepBalanceOrg.sub(oneToken)).toString()).should.be.equal(orgBalanceRep.toString());
                        assert.equal(memberBalancePenalties.toString(), zero.toString());
                        assert.equal(orgBalancePenalties.toString(), amount.toString());
                    });
                    it(' and emits a "ReputationBurned" and "PenaltiesMinted" event', async function () {
                        const tx = await eeaOperatorInstance.mintPenalties(org, to, amount, '0x0', {
                            from: eeaAdmin
                        });
                        // Test the event
        ...
        describe('when the sender is not the EEA issuer', function () {
            it('reverts', async function () {
                await expectThrow(eeaOperatorInstance.mintPenalties(org, to, amount, '0x0', {
                    from: unauthorizedMember
                }));
            });
        });
